	...
	\begin{document}
		...
		A aplicação uRLLC é usada para transportar alertas dos sensores com o menor tempo possível, enquanto o eMBB é colocado para o envio de um volume maior de dados associado às imagens das câmeras. Como os dois tipos de tráfego compartilham a mesma rede de tráfego, a carga gerada pelo eMBB pode afetar a latência e a entrega das mensagens uRLLC quando não existe classificação de prioridade.
		...
		A arquitetura geral da rede emulada é apresentada na Figura~\ref{fig:arquitetura}.
        ...
        O primeiro cenário testado foi o isolado. Nele, apenas o tráfego uRLLC está ativo, sem concorrer com o eMBB; somente os sensores transmitem dados para a central. Com isso, a latência se mostrou abaixo de 5 ms em todos os casos. Os resultados agregados de conformidade do tráfego uRLLC são apresentados na Tabela~\ref{tab:conformidade-urllc}.
		...
		A Figura~\ref{fig:metricas-urllc} detalha visualmente as perdas, os \textit{timeouts} e a não conformidade efetiva por cenário.
		...
		A distribuição acumulada da latência uRLLC nos quatro cenários é apresentada na Figura~\ref{fig:cdf-urllc}.
		...
		A Tabela~\ref{tab:vazao-embb} e a Figura~\ref{fig:vazao-embb} evidenciam que o \textit{closed loop} protege o tráfego uRLLC à custa de uma redução acentuada da vazão eMBB, confirmando o compromisso entre latência e vazão.
		...
		A avaliação foi realizada com quatro cenários, cada um submetido a cinco repetições de 60 segundos. A não conformidade efetiva foi de 0,08\% no cenário isolado, 45,55\% no cenário sem QoS, 50,39\% no QoS estático e 31,33\% no \textit{closed loop}. Entre os pacotes recebidos, o \textit{closed loop} apresentou p95 de 3,233 ms; entretanto, ainda foram observados 29,64\% de perdas ou \textit{timeouts}. Portanto, a redução da não conformidade não representa uma garantia de que todas as tentativas atenderam ao limite de 5 ms.

		Os resultados também mostraram um compromisso entre a proteção do uRLLC e o desempenho eMBB. No \textit{closed loop}, a vazão eMBB foi reduzida para 0,932 Mbit/s, com 92,27\% de perdas. Assim, o mecanismo demonstrou capacidade de reduzir a latência dos pacotes uRLLC recebidos sob concorrência, mas ainda precisa ser interpretado como uma estratégia de proteção parcial, cujo custo sobre o tráfego eMBB deve ser considerado. Como os experimentos foram realizados em ambiente emulado, os resultados caracterizam as condições avaliadas no laboratório e não constituem garantia de desempenho em uma rede 5G real. De modo geral, os quatro nós OVS representam uma abstração de camada 2 (L2) do plano de encaminhamento; portanto, não correspondem a roteadores IP completos. O relógio compartilhado da VM facilita a medição do atraso; no entanto, em uma rede física, seria necessária sincronização por PTP/NTP. O Docker Desktop e o \textit{datapath} em \textit{userspace} podem introduzir \textit{jitter}. Além disso, uma única execução não é suficiente para sustentar conclusões estatísticas.
