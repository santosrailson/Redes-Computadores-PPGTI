	...
	\begin{document}
        ...
        O tráfego da rede uRLLC que passam as mensagens TCP pela porta 5000 e eMBB onde passa o fluxo relacionado a vídeo é gerado pelo o iperf3 através das portas 5001-5003, são simulações onde atesta os resultado e objetivos alcançados da proposta do projeto. Um mecânismo importante nessa construção é o QoS (Qualidade de Serviço), o qual cria filas, onde a fila 0 representa o tráfego uRLLC e a fila 1 representa o eMBB,  e essas regras são implementadas graças ao OpenFlow dentro dos switchs OVS. E como funciona? a fila uRLLC tem banda reservada e o eMBB a banda é ajustável pois não tem prioridade como a anterior.

        Para assegurar o monitoramento é utilizado o closed loop, é com ele que o ciclo segue a seguinte sequência: a central observa cada mensagem uRLLC e mede a latência; o controlador toma a decisão de medir o limite de 5 ms, e se esse limite ulltrapassar o máximo, reduz a taxa máxima das filas eMBB de 20 para 2 Mbit/s aliviando a rede para o uRLCC. Todo esse processo é repetido e verificado graças ao closed loop para manter uma rede estável principalemnte para os sensores.

        No caso do tráfego eMBB ser limitado ou ajustado para priorizar o tráfego uRLLC, é para o tráfego das câmeras não ser interrompido e manter a rede completamente ativa.
		...
		HTB \cite{olegario2024qos5g,linuxhtb} configurada em cada porta OVS, com duas filas:
		...
		mediano (DAM). Indique as condi\c{c}\~oes comparadas:
		sem controle (baseline) e Abordagem B (OVS/OpenFlow com QoS/HTB).
		...
		distribui\c{c}\~ao acumulada da lat\^encia one-way para as
		condi\c{c}\~oes baseline e OVS/OpenFlow em uma \'unica figura;
		inclua uma linha tracejada vertical no limiar de 5\,ms para
		facilitar a leitura. A
		...
		Abordagem B (OVS/OpenFlow) reduz as viola\c{c}\~oes de lat\^encia:
		as regras de fluxo s\~ao instaladas diretamente nos switches via
		OpenFlow, e as filas HTB garantem prioridade m\'axima ao tr\'afego
		uRLLC \cite{olegario2024qos5g,pfaff2015ovs,linuxhtb}. Compare com a baseline (sem
		controle): sem a fila de alta prioridade, o tr\'afego eMBB
		satura o enlace e aumenta a lat\^encia do uRLLC. Discuta as
		limita\c{c}\~oes dos resultados: o OVS executa em software, portanto
		os valores absolutos de lat\^encia medem a emula\c{c}\~ao e n\~ao
		devem ser comparados diretamente com hardware de produ\c{c}\~ao.
		Aponte tamb\'em que o gerador uRLLC utiliza Scapy sobre TCP,
		o que introduz overhead de processamento nos hosts terminais.
